...
- [5 - Interface de usuário](#5-interface-de-usuário)
  - [5.1. Páginas principais:](#51-páginas-principais)
    - [5.1.1. Página de login](#511-página-de-login)
    - [5.1.2. Página de locais/varreduras: visualizar todos os locais e varreduras realizadas](#512-página-de-locais/varreduras:-visualizar-todos-os-locais-e-varreduras-realizadas)
    - [5.1.3. Página de varreduras em um local: explorar todas as varreduras realizadas em um local específico](#513-página-de-varreduras-em-um-local:-explorar-todas-as-varreduras-realizadas-em-um-local-específico)
    - [5.1.4. Página de detalhamento de varredura: explorar as informações detalhadas de uma varredura específica](#514-página-de-detalhamento-de-varredura:-explorar-as-informações-detalhadas-de-uma-varredura-específica)
    - [5.1.5. Página de realização de nova varredura](#515-página-de-realização-de-nova-varredura)
...
<p align="center"><img src="https://github.com/2023M6T2-Inteli/Inspectron/assets/99221221/108b3e6e-a809-45ea-bc07-90aaccc1beec" width=120%></img></p>
...
1. **Embarcado** : Esta parte está presente no próprio robô e engloba todo o controle de movimento e os sensores utilizados, como IMU (Unidade de Medição Inercial), LIDAR (Sensor de Detecção e Alcance de Luz) e Câmera. Esses dispositivos são responsáveis por capturar informações do ambiente e fornecer dados essenciais para o funcionamento autônomo do robô. Esses sensores trabalham em conjunto para fornecer ao robô informações essenciais sobre o ambiente circundante. Os dados capturados pelos sensores são processados e interpretados pelo sistema de controle do robô, permitindo que ele tome decisões autônomas sobre seu movimento, evite obstáculos e realize as tarefas de inspeção de forma eficiente.A integração desses sensores no sistema de percepção e controle é fundamental para o funcionamento autônomo do robô de inspeção de plantas, permitindo que ele navegue de forma segura e eficaz pelo ambiente e colete os dados necessários para a análise das estruturas.
2. **Backend** : O sistema de servidor em nuvem constitui o backend da nossa solução. Ele é responsável por fornecer dados provenientes do embarcado para o frontend, bem como armazenar as varreduras realizadas pelo robô e o código necessário para a execução dos trajetos autônomos. O backend é crucial para o processamento e armazenamento eficiente das informações coletadas, garantindo acesso rápido aos dados relevantes quando necessário. O sistema de servidor em nuvem é essencial para a eficiência, processamento de dados e armazenamento adequado do robô de inspeção de plantas. Ele permite a comunicação entre o frontend e o robô, bem como o processamento e a análise dos dados coletados para tomada de decisões informadas e geração de relatórios úteis.
3. **Frontend** : O frontend consiste no principal painel de controle utilizado pelo usuário. Através dele, são apresentados dados sobre as rotinas e funcionalidades do robô. O frontend também permite ao usuário enviar solicitações e comandos indiretos ao robô, utilizando o backend para estabelecer comunicação por meio de requisições HTTP ou Sockets. Esse ambiente de interação intuitivo e amigável permite ao usuário monitorar e controlar o robô remotamente. O frontend desempenha um papel central como o painel de controle principal utilizado pelo usuário para interagir com o robô de inspeção de plantas. Ele é uma parte essencial da solução de robô de inspeção de plantas, fornecendo uma interface interativa para o usuário monitorar, controlar e interagir com o robô de forma eficiente e intuitiva.
...
### **5.1.2. Página de locais/varreduras: visualizar todos os locais e varreduras realizadas**
...
### **5.1.3. Página de varreduras em um local: explorar todas as varreduras realizadas em um local específico**
...
### **5.1.4. Página de detalhamento de varredura: explorar as informações detalhadas de uma varredura específica**
...
### **5.1.5. Página de realização de nova varredura**
...
## 7.3. Implementações realizadas

A seguir, entraremos em mais detalhes sobre nossas implementações no âmbito de dispositivos de segurança. Nossos focos principais foram em relação à movimentação do robô, gostariamos de implementar features que aumentassem a confiabilidade do produto para a realização deste serviço. Decidimos por utilizar de sensores como o LIDAR para garantir a movimentação fluida e segura do robô e pela implementacao de um sistema de parada de emergencia podendo ser ativada a todo momento pelo usuário durante a varredura.

### 7.3.1. Sensor Lidar

A detecção de obstáculos é uma etapa fundamental para garantir que o robô mapeie os locais da melhor maneira possível, evitando colisões e interações indesejadas com objetos presentes no ambiente. O sensor Lidar é capaz de fornecer informações precisas sobre a proximidade de objetos em torno do robô, permitindo que ele tome decisões de navegação adequadas para evitar possíveis danos a si mesmo e ao ambiente. Atualmente, nossa aplicação dispõem de uma análise constante em quanto em movimento de obstruções diante do robô e automatizamos no script seu desvio. Logo, nos organizamos para utilizar deste sensor em prol da segurança e efetividade nas inpeções que o robô deverá realizar.

### 7.3.2 Parada de Emergência

A fim de evitar acidentes no percuso, implementamos um botão de facil acesso no front para o usuário com o fim de possibiliar a realização da parada de emergência caso necessário. Esta é fundamental para evitar possiveis acidentes e danos ao robô ou à estrutura no qual esta inserido. Pensando no contexto do projeto, basicamente, ao iniciar uma nova varredura e obter as respostas do robô (sensores/imagens), nós tambem iniciamos sua escuta no tópico ao qual sera realizado a postagem caso o usuário selecione o botão de parada apresentado, o robô em seguida deverá parar sua movimentacao e serviços e aguardar novas instruções.
...
* tvoc: Esse tópico é resposável por receber dados do sensor CJMCU-811 acoplado ao robô. Uma das funções desse sensor é medir e monitorar a presença e concentração de compostos orgânicos voláteis no ambiente.
* eco2: Esse tópico também é resposável por receber dados do sensor CJMCU-811 acoplado ao robô. Diferente do tvoc, esse tópico envia dados relativos a quantidade de  dióxido de carbono presente no ambiente.
...
* gps: Esse tópico é resposável por receber dados do sensor 'GPS GY-NEO6MV2' acoplado ao robô. Uma das funções desse sensor é coletar a latitude e longitude de sua posição atual para fornecer a lozalização em caso de perda de conexão.
* temperature:Esse tópico é resposável por receber dados da temperatura do ambiente usando o sensore "DHT11", 
