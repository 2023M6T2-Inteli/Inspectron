...
A partir da necessidade de realizar um serviço de streaming entre o robô e a plataforma, bem como a implementação de um modelo pré-treinado para a avaliação de rachaduras nas imagens captadas ao longo do percurso, desenvolvemos um sistema de visão computacional para cumprir tais objetivos.

Para o serviço de streaming, utilizamos tecnologias como Socket.io, que nos permite subscrever e publicar em tópicos da rede local. A fim de transmitir as imagens pela rede, foi necessário converter as imagens obtidas em um formato adequado. Para isso, utilizamos a função <base64.b64encode(SEU_FRAME)>, que converte uma sequência de bytes em uma sequência de caracteres base64. Essa conversão permite que os frames estejam em um formato que possa ser enviado via tópicos. Nesse caso, criamos o tópico 'streaming' para essa finalidade.

Seguindo a lógica da aplicação, nosso servidor Socket.io está ouvindo o tópico 'streaming' no qual o nodo ROS publica as imagens, e pode emitir essa mensagem (frame) para o frontend. Em ambos os passos, a imagem é convertida e reconvertida, sendo que em um momento é salva em um arquivo apenas para relatório da inspeção, e em outro momento é transmitida para o serviço de streaming final.

Realizamos testes do modelo preditivo disponível pela YoLo para análise de imagens. Utilizamos a câmera de um notebook para testar a detecção de rachaduras. Colocamos imagens diante da câmera que poderiam ser detectadas e, na maioria dos casos, o modelo apresentou corretamente as dimensões e posições das rachaduras na tela. Isso nos permitiu concluir que o modelo foi treinado corretamente e que podemos utilizar o arquivo .pt gerado pelo modelo em nosso backend para realizar a inspeção em imagens recebidas pelo nodo ROS e apresentá-las imediatamente no frontend.

Para testar o serviço de streaming, utilizamos scripts Python do Socket e do robô. Utilizamos a biblioteca cv2 para capturar a imagem da webcam do notebook, com apenas uma pequena modificação para utilizar a câmera instalada no microcomputador do robô. Além disso, utilizamos a biblioteca base64 para realizar as devidas conversões dos frames para a transmissão adequada. Com isso, foi possível testar e verificar o funcionamento do serviço de streaming.
